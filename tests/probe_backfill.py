"""
Prueba el endpoint de edición de presencia para fechas pasadas.
Usa 2026-02-25 (miércoles pasado) como test.
"""
import requests, os, base64, json, time

DOMAIN   = os.environ.get("WOFFU_DOMAIN", "gtd.woffu.com").replace("https://","").strip("/")
USER     = os.environ.get("WOFFU_USER", "")
PASSWORD = os.environ.get("WOFFU_PASS", "")
BASE     = f"https://{DOMAIN}"
PROXIES  = {k: v for k, v in {
    "http":  os.environ.get("HTTP_PROXY")  or os.environ.get("http_proxy"),
    "https": os.environ.get("HTTPS_PROXY") or os.environ.get("https_proxy"),
}.items() if v}

# Login
r = requests.post(
    "https://app.woffu.com/api/svc/accounts/authorization/token",
    data={"grant_type": "password", "username": USER, "password": PASSWORD},
    headers={"Content-Type": "application/x-www-form-urlencoded"},
    timeout=15, proxies=PROXIES,
)
token = r.json().get("accessToken")
p     = json.loads(base64.urlsafe_b64decode(token.split(".")[1] + "=="))
uid   = int(p.get("UserId"))
print(f"✅ Login OK. UserId={uid}\n")

hdrs = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

# ── TEST: editar presencia de un día pasado ───────────────────
TEST_DATE  = "2026-02-24"   # lunes pasado — cámbialo si ese día tienes ausencia
ENTRY_TIME = "09:32:00"
EXIT_TIME  = "18:33:00"

slot_id = str(int(time.time() * 1000))  # timestamp en ms, como hace la web

payload = {
    "date":     TEST_DATE,
    "comments": "",
    "userId":   uid,
    "slots": [
        {
            "id":    slot_id,
            "motive": None,
            "in": {
                "new":              True,
                "deleted":          False,
                "agreementEventId": None,
                "code":             None,
                "iP":               None,
                "requestId":        None,
                "signId":           0,
                "signStatus":       1,
                "signType":         3,
                "time":             ENTRY_TIME,
                "deviceId":         None,
                "signIn":           True,
                "userId":           0,
            },
            "out": {
                "new":              True,
                "deleted":          False,
                "agreementEventId": None,
                "code":             None,
                "iP":               None,
                "requestId":        None,
                "signId":           0,
                "signStatus":       1,
                "signType":         3,
                "time":             EXIT_TIME,
                "deviceId":         None,
                "signIn":           False,
                "userId":           0,
            },
            "order": 1,
            "totalMin": 541,
        }
    ],
}

print(f"📅 Probando PUT para {TEST_DATE} ({ENTRY_TIME} - {EXIT_TIME})...")
r = requests.put(
    f"{BASE}/api/svc/core/users/{uid}/diarysummaries/workday/slots/self",
    json=payload,
    headers=hdrs,
    timeout=15, proxies=PROXIES,
)
print(f"  Status: {r.status_code}")
print(f"  Respuesta: {r.text[:400]}")

if r.status_code == 200:
    print(f"\n✅ Funciona! Revisa en Woffu que {TEST_DATE} muestra {ENTRY_TIME} - {EXIT_TIME}")
else:
    print(f"\n❌ No funcionó. Puede que el día ya tenga fichaje o que el formato sea distinto.")